...
- 富文本边界（2026-09 起 WYSIWYG）：主帖正文与层内容（含编辑）用 `components/editor/rich-editor.tsx`（TipTap：加粗/斜体/删除线/行内代码/代码块/H2/H3/引用/列表/链接/贴吧表情/撤销重做）；层内回复及其编辑仍仅普通文字（`components/forum/thread-view.tsx` 按 `reply.floorNo>0` 分支）。
