...
- 已实现公告、站点设置、板块、标签、表情资源、举报队列与处理的管理 API/UI，并记录审计日志。任何新的管理写操作必须校验 `getAdminUser(request)` 且记审计（见 PRD MOD-002）。
...
- 管理员审计日志与表情资源管理已实现（管理写操作记录动作、目标与时间）。
...
- 运行时 SQL 接口、`npm run db:migrate` 与 GitHub Actions 空库迁移矩阵已支持 Node 下的 PostgreSQL/MySQL 驱动选择（`DATABASE_DRIVER` + `DATABASE_URL`）；并发语义校验仍未实现。
