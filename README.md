...
「无名岛」是一个不需要手机号或邮箱即可使用的匿名讨论社区：注册一个用户名即可进入（登录页 `/login`），浏览与发言都需登录；每条帖子与回复可选「匿名发言」（同一帖子内显示随机代号 匿名 A1、A2…，跨帖不暴露固定身份）或「以用户名发言」（跨帖展示固定用户名与唯一 ID）。阅读位置自动保存，下次打开可一键续读。
...
- **登录门槛与独立登录页**：浏览与发言均需登录（`/login` 独立登录/注册页，`?next=` 深链回跳）；退出登录自动回到登录页；注册无需邮箱/手机号，用户名 + 密码即获唯一 ID（#编号）
...
- **帖子详情** `/t/[id]`：楼层与层内回复严格分层——楼层号只分配给直接回复楼主的层内容（1 楼=主帖，2、3…顺延），回复某层的“层内回复”不占楼号、折叠展示；主帖/楼层内容支持受限富文本（加粗、斜体、行内代码、代码块、引用、链接、**百度贴吧表情包**、预览），层内回复仅支持普通文字
  - 表情资源说明：104 个百度贴吧官方默认表情已自托管于 `public/emoji/tieba/`（避免外链失效），正文以标准图片语法 `![贴吧表情 N](/emoji/tieba/image_emoticonN.png)` 存储，渲染层仅放行该路径白名单；图片版权归百度所有，如需移除请删除该目录并同步 `lib/tieba-emojis.ts`
...
- **默认超级管理员**：`admin / admin123`（角色 `super_admin`，与普通注册用户分表分会话；管理 API 见 `/api/v1/admin/login|logout|me`）。仅存 PBKDF2 哈希，**上线前必须修改默认密码**；管理控制台页面尚未实现
