...
const safeErrors: Record<string, { status: number; message: string }> = {
  ...
  MEDIA_FILE_INVALID: { status: 400, message: '仅支持 5MB 以内且内容有效的 JPG、PNG 或 WebP 图片' },
  MEDIA_UPLOAD_NOT_FOUND: { status: 404, message: '上传任务不存在或不属于当前匿名身份' },
  MEDIA_UPLOAD_EXPIRED: { status: 410, message: '上传任务已过期，请重新选择文件' },
  MEDIA_UPLOAD_STATE: { status: 409, message: '上传任务状态不允许重复上传' },
  MEDIA_UPLOAD_NOT_READY: { status: 409, message: '文件尚未上传完成，请稍后重试' },
  EMOJI_NOT_FOUND: { status: 404, message: '表情不存在或已下架' },
  ...
  ADMIN_CURRENT_PASSWORD_INVALID: { status: 400, message: '当前管理员密码不正确' },
